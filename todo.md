# Yeniləmə yoxlama siyahısı

- [x] Layihənin funksiyalarını və quraşdırma tələblərini README üçün yoxlamaq.
- [x] İngilis dilli README sənədini yazmaq.
- [x] Məxfi GitHub repozitoriyası yaratmaq və layihəni yükləmək.

- [x] Brauzer konum icazəsi və koordinat əsasında hava sorğusunu tətbiq etmək.
- [x] Konum icazəsi rədd ediləndə və əlçatan olmayanda aydın xəta mesajı göstərmək.
- [x] Konum funksiyasını masaüstü və mobil görünüşdə yoxlamaq.

- [x] Mövcud mətnləri Azərbaycan və İngilis dili lüğətinə çıxarmaq.
- [x] Dil seçimi idarəsini və seçimin yadda saxlanmasını əlavə etmək.
- [x] Göyçay üçün geocoding nəticəsini yoxlamaq və etibarlı ehtiyat xəritələməsi qurmaq.
- [x] API-dən qaytarılan cari hava oxunuşlarının canlılıq vəziyyətini yoxlamaq.
- [x] Axtarış, xəta halları və mobil görünüşü yenidən test etmək.
